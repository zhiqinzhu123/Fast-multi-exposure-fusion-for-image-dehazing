...
% set wSize  
[imagename1 imagepath1]=uigetfile('demo_images/*.jpg;*.bmp;*.png;*.tif;*.tiff;*.pgm;*.gif','Please choose the first input image');
I1=imread(strcat(imagepath1,imagename1));  
% I1=double(imgSeqColor(:,:,:,1))/255;
% figure,imshow(I1l);
% figure,imshow(I1h);
[imagename2 imagepath2]=uigetfile('demo_images/*.jpg;*.bmp;*.png;*.tif;*.tiff;*.pgm;*.gif','Please choose the first input image');
I2=imread(strcat(imagepath2,imagename2));  
% I2=double(imgSeqColor(:,:,:,2))/255;
tic
% I1 = imnoise(I1,'gaussian',0.02); %gauss
% I2 = imnoise(I2,'gaussian',0.02); %gauss
% imshow(I1);
num=2;
flag=0;
k=8;%¿é
...
mu=10;
% patch size set
 [I11,I22,I1h,I1l,I2h,I2l,I2thog,I1thog]=CT_DE(I1,I2,lambda,mu,para2);
% %     default_wSize = 8;
% %     default_wSize = 16;
%  imwrite(I1h,'image/A.tif');
%  imwrite(I2h,'image/B.tif');
 Ia = imread('./image/A.tif');
 Ib = imread('./image/B.tif');
   default_wSize = 21;
%     figure,imshow(I1h);
%     figure,imshow(I2h);
    [D]=SHANG(Ia);
    ...
    C=(D+G)/2;
% %     a=0.6;
% %      default_wSize=round(default_wSize+(C-7)*C+(C-5)-(C-3)*C*C);  
     b=(C/10).^(C)-((-C)/10).^(-C);
 c=(C/10).^(C)+((-C)/10).^(-C);
default_wSize=round((default_wSize/8)*(b/c)+(default_wSize)*exp((-C)*0.1));
%     if (C<4)
%         if((2<C)&&(C<3))            
%          default_wSize =round(3*C) ;  
% %           default_stepSize =floor(default_wSize/4);
%         else
%             if((3<=C)&&(C<4));
%                default_wSize =round(default_wSize/2+3*round(C-2)*2);
%             else
%           default_wSize =floor(default_wSize/8)+round(2*C);
%             end
% %            default_stepSize = floor(2*default_wSize);
%         end
%         else
%         if ((4<=C)&&(C<7))
%         default_wSize = floor(default_wSize)+round(C);
% %          default_stepSize = floor(default_wSize/10);
%         else
%           default_wSize = default_wSize+round(2*C);
% %            default_stepSize = floor(default_wSize/10);
%         end
%     end
